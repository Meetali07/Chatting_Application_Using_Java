...
public class Server  implements ActionListener{
    ...
    JPanel a1; // Declare a JPanel for the main content area globally
    static Box vertical=Box.createVerticalBox(); // Create a vertical box layout for the main content area for text messages
    static JFrame f=new JFrame(); //there was error in validate function bez it wasnot static so thats why to make JFrame static we created a class otherwise before we had called extended it above
    // Constructor for the Server class
    static DataOutputStream dout;

    Server() {                                      // Constructor logic added here
        f.setLayout(null); 
        ...
        ImageIcon i4 = new ImageIcon("Icons/1.jpg"); // Load the server icon
        ...
        f.setLocation(200,50);                    // Set the location of the window coordinates x and y axis 
    ...
    public void actionPerformed(ActionEvent ae) { //you are overriding the method from the ActionListener interface. This means you are providing your own code that will run whenever an action event (like a button click) occurs and this listener is registered.
        ...
        try{
            ...
            dout.writeUTF(out); // Send the message to the client using DataOutputStream
    ...
    public static void main(String[] args) {
        ...
        new Server();
        try{
            ServerSocket skt=new ServerSocket(6001); // Create a new ServerSocket on port 6001
            while(true){ // infinte loop top accept the messages
                Socket s=skt.accept(); // Accept incoming connections
                ...
                dout=new DataOutputStream(s.getOutputStream()); // Create a DataOutputStream to send data to the socket
